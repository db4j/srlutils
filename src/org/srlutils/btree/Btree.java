    ...
    implements Bface<CC>
{
    ...
    public abstract PP rootz(PP page,CC context);
    /** set the map depth and update context.depth */
    public abstract void depth(int level,CC context);
    ...
    public Path findPath(CC context,boolean get) {
        ...
        boolean right = true, prior = false;
        while (true) {
            context.match = false;
            ko = findIndex( page, context.mode, context );
            Path p3 = path;
        ...
        if (ko < page.num && (!modes.eq(context.mode) | compare(page,ko,context)==0)) {
            context.match = true;
            if (get) getcc(page, context, ko);
        }
        return path;
    }
    public void verify(CC cc) {
        PP page = rootz(null,cc);
        if (cc.depth > 0) verify(page,0,cc,null);
    }
    public void verify(PP page,int level) {
        CC cc = context();
        getcc(page,cc,0);
        initContext(cc);
    ...
    public void verify(PP page,int level,CC cc,CC cp) {
    ...
    public void insert2(CC context) {
...
//        System.out.format( "Btree.remove -- page:%5d num:%5d, %5d %5d %5d\n", page.kpage, page.num, size, s2, so );
    ...
    public Range<CC> getall() { return getall(context()); }
    ...
    public static class Range<CC extends Btree.Context> {
        ...
        public boolean next() {
            ...
            first = false;
            if (valid) btree.getcc(p1.page,cc,p1.ko);
            return valid;
        }
    }
    ...
    /** return the path to the end of the map */
    ...
    public PP dexs(PP parent,int index,boolean leaf,CC cc) {
    ...
    public static abstract class DirectMap<CC extends Context>
        ...
        public int depth;
        public Sheet rootz(Sheet page,CC context) {
        ...
        public void depth(int level,CC context) {
    ...
    static class Tester extends TaskTimer.Runner<Void> {
        boolean ok;
        public void alloc() { stageNames = "put look rem chk".split(" "); setup(stageNames.length, "DF"); }
        public boolean finish() { return ok; }
        DF map = new DF();
        Data cc;
        double [] keys;
        int nn;
        public Tester(int $nn) { nn = $nn; }
        public void init() { keys = Rand.rand(nn+1); cc = map.context(); map.init(cc); ok = true; }
    ...
    static class Demo {
        public static void main(String [] args) throws Exception {
            TestDF.auto(null,1000000,1,1,new Tester(1000000));
            TestDF.auto(null,1000000,3,3,new Tester(1000000));
        }
    }
